...
for i in range(20):
    ...
    os.makedirs(f'edge_and_weighted_degree', exist_ok=True)
    pd.DataFrame(np.concat([np.array(weights),np.array(deg_w)])).to_csv(f'edge_and_weighted_degree/{i}.csv', index=False, header=False)