...
def get_grad_var(weights, edges, cost_hamiltonian):
    ...
    for i in range(NUM_TRIAL):
        ...
        grad_list.append(get_grad(weights, edges, cost_hamiltonian, i))
...
for i in range(20):
    ...
    os.makedirs(f'grad_var_all', exist_ok=True)
    pd.DataFrame(np.array(grad_var)).to_csv(f'grad_var_all/{i}.csv', index=False, header=False)